 ...
 * - Writes ./autofix-status.json, a job summary line, and updates/creates "Autonomy Status" issue.
...
const RUN = {
  ...
  provider: "none",
  providersTried: [],   // entries like "gemini:present http=200 parsed=false reason=no-json"
  smokeSeen: false,
  bootstrapChanges: 0,
  llmChanges: 0,
  branch: "",
...
async function callHF(prompt) {
  ...
  const tag = `huggingface:${token ? "present" : "missing"}`;
  RUN.providersTried.push(tag);
...
${prompt}`,
  ...
  logd("hf http", r.status);
  if (!r.ok) { RUN.notes.push(`hf http=${r.status}`); return null; }
  const data = await r.json();
  ...
  const out = parseJSON(m ? m[0] : text);
  if (!out) RUN.notes.push("hf parsed=false (no strict JSON)");
...
async function callGroq(prompt) {
  ...
  const r = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    ...
    body: JSON.stringify({
      model,
      messages:[
        { role:"system", content:"Output STRICT JSON only: {\"files\":[{\"path\":\"Backend/...\",\"content\":\"...\"}]}\\nTouch only Backend/. Keep patches small; no new deps." },
...
async function main() {
  ...
  try {
    ...
    if (resp && Array.isArray(resp.files)) {
      ...
      RUN.notes.push("no LLM patch applied (keys missing/unavailable or non-JSON)");
    }
  } catch (e) {
