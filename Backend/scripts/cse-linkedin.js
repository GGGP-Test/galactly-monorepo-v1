// Simple, ASCII-only generator for LinkedIn RSSHub company feeds.
// Run with Node 18+ (Node 20+ recommended). No imports, no packages.
// Example (one line):
//   node scripts/cse-linkedin-simple.js --api-keys KEY1 --cxs CX1 \
//     --base https://YOUR-RSSHUB.onrender.com --access-key YOUR_RSSHUB_KEY \
...
if(!API_KEYS.length || !CXS.length){
  console.error("[cse] Missing --api-keys or --cxs");
  process.exit(1);
}
if(!BASE || !KEY){
  console.error("[cse] Missing --base or --access-key");
  process.exit(1);
}

// Seed terms (ASCII only)
...
function geoTerms(){
  if (GEO === "US") return ["United States","USA"].concat(US_STATES);
  if (GEO === "CA") return ["Canada"].concat(CA_PROVINCES);
  if (GEO === "ALL") return [];
...
(async function main(){
  ...
  console.error("[cse] queries="+queries.length+", pages="+PAGES_PER_QUERY);
      ...
      try{
        ...
        for(const it of items){
          ...
          if(feeds.length >= MAX_RESULTS){
            console.error("[cse] max reached="+MAX_RESULTS);
            break outer;
          }
        }
      }catch(e){ /* ignore to keep going */ }
  ...
  console.log(line);
  console.error("[cse] emitted="+feeds.length);
})();
