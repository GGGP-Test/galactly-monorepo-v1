...
function norm(x:number,min:number,max:number,fallback=0.5){
  if(!Number.isFinite(x))return fallback; if(max<=min)return fallback;
...
async function main(){
  ...
  await q('UPDATE model_state SET weights=$1, updated_at=now() WHERE segment=$2', [weights, 'global']);
