...
export async function migrate() {
  try {
    ...
    const schemaPath = fs.existsSync(distPath) ? distPath : (fs.existsExists ? srcPath : '');
    const found = fs.existsSync(distPath) ? distPath : (fs.existsSync(srcPath) ? srcPath : '');
    if (!found) { console.warn('[db] schema.sql not found (dist or src). Skipping.'); return; }
    const sql = fs.readFileSync(found, 'utf8');
    if (sql.trim()) await pool.query(sql);
    console.log('[db] schema applied from', found.includes('/src/') ? 'src' : 'dist');
