 ...
 *  - Deterministic tokens for joins across systems without revealing PII (HMAC-SHA256).
 *  - Role/tenant scoped access checks + audit trail hooks.
 *  - Redaction utilities for logs/exports.
 *
 * Wire this to your DB layer; by default this exports in-memory stubs you can replace.
 */
...
export class LocalKeyManager implements KeyManager {
  constructor(private masterKey = process.env.PII_MASTER_KEY || "dev-only-master-key-please-override") {}
...
export class InMemoryVaultStore implements VaultStore {
  ...
  async purgeByTenant(tenantId: string): Promise<number> {
    ...
    for (const k of Array.from(this.map.keys())) {
      if (k.startsWith(`${tenantId}:`)) { this.map.delete(k); count++; }
    }
    return count;
  }
}
...
export class PIIVault {
  ...
  constructor(private store: VaultStore, private keys: KeyManager, pepper = process.env.PII_TOKEN_PEPPER) {
    const p = pepper || "dev-only-token-pepper-override";
    this.pepper = Buffer.isBuffer(p) ? p : Buffer.from(p, "utf8");
  }
  ...
  tokenize(kind: PIIKind, value: string, tenantId: string): string {
    ...
    const mac = crypto.createHmac("sha256", Buffer.concat([this.pepper, Buffer.from(tenantId, "utf8")]));
    mac.update(kind);
    mac.update("|");
    mac.update(norm);
    return mac.digest("base64url"); // URL-safe compact token
  }

  /**
   * Encrypt and store the raw PII under deterministic token.
   */
  async vaultPut(ctx: AccessContext, kind: PIIKind, value: string): Promise<{ token: string }> {
  ...
  /**
   * Pseudonymous export: expose tokens only (no decryption).
   */
...
function normalize(kind: PIIKind, value: string): string {
  ...
  if (kind === "phone") {
    ...
    return d.length === 10 ? d : d; // keep as-is after digits
  }
  return value.trim();
}
...
function requireAccess(ctx: AccessContext, roles: AccessContext["role"][], purposes: AccessContext["purpose"][]) {
