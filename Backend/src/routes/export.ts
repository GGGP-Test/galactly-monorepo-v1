...
//
// Lead export (CSV) with web/catalog/hybrid sources.
// - Gated like /api/web/find-buyers: free cannot request HOT unless admin
// - Mirrors tier/size params used elsewhere
// - No external deps; safe in Docker slim
//
// Endpoints:
//   GET /api/export/ping
//   GET /api/export/csv?host=acme.com
//       [&city=&size=small|medium|large&limit=50]
//       [&band=COOL|WARM|HOT]
//       [&mode=web|catalog|hybrid&shareWeb=0.4]
//       [&tiers=A,B,C]&[preferTier=A|B|C]&[preferSize=small|medium|large]
//
// Notes:
//   - Sends text/csv with a filename hint
//   - Fields: host,name,url,city,tier,score,band,uncertainty,provider,tags,reasons
//
// Mount in index.ts:
...
// Simple admin/plan checks (mirror of leads-web)
...
function asCsv(rows: Array<Record<string, unknown>>, cols: string[]): string {
...
r.get("/csv", async (req: Request, res: Response) => {
  const t0 = Date.now();
  try {
    ...
    const limitQ = Math.max(1, Math.min(1000, Number(req.query.limit ?? 100) || 100));
    ...
    // plan gating: Free cannot export HOT unless admin
    ...
    const shareWeb = Math.max(0, Math.min(1, Number(req.query.shareWeb ?? 0.4)));
    ...
    // ---------- CSV ----------
    const cols = [
      "host","name","url","city","tier","score","band","uncertainty","provider","tags","reasons"
    ];
    const rows = items.map((it) => ({
      host: it.host,
      name: it.name || "",
      url: it.url || `https://${it.host}`,
      city: it.city || "",
      tier: getTier(it),
      score: typeof it.score === "number" ? it.score.toFixed(1) : "",
      band: it.band || "",
      uncertainty: typeof it.uncertainty === "number" ? it.uncertainty.toFixed(3) : "",
      provider: it.provider || "",
      tags: (Array.isArray(it.tags) ? it.tags : []).slice(0, 12).join(";"),
      reasons: (Array.isArray(it.reasons) ? it.reasons : []).slice(0, 12).join(" | "),
    }));

    const csv = asCsv(rows, cols);
    const fname = `export_${host}_${band.toLowerCase()}_${mode}_${Date.now()}.csv`;
    ...
    try {
      ...
      await F(url, {
        ...
        body: JSON.stringify({
          ...
          data: {
            host, plan, adminOverride: admin, bandRequested: requestedBand, bandApplied: band,
            mode, shareWeb: mode === "hybrid" ? shareWeb : (mode === "web" ? 1 : 0),
            returned: items.length, ms: Date.now() - t0,
          },
        }),
      });
    } catch {}
    ...
    return res.status(200).json({ ok: false, error: "export-csv-failed", detail: msg });
  }
});
