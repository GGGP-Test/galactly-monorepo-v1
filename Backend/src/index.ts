...
let prefs: UserPrefs | undefined = undefined;
if (userId) {
...
const demos = Array.from({ length: missing }).map((_, i) => ({ id: -(i + 1), cat: 'demo', kw: ['packaging'], platform: 'demo', fit_user: 60, heat: 60, source_url: 'https://example.com/demo', title: 'Sample Lead', snippet: 'Demo card while ingest warms up', ttl: toISO(nowPlusMinutes(60)), state: 'available', created_at: new Date().toISOString(), _score: 0 }));
leads = [...leads, ...demos];
}
