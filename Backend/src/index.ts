...
app.post('/api/v1/claim', (req, res) => {
  ...
  const reservedUntil = Date.now() + 60_000,
    decisionDeadline = Date.now() + 5 * 60_000;
  const ok = db
    .prepare(`UPDATE lead_pool SET state='reserved', reserved_by=?, reserved_until=? WHERE id=? AND state='available'`)
...
app.post('/api/v1/arrange-more', (req, res) => {
  ...
  if (lead) {
    ...
    const prefs =
      (db
        .prepare(`SELECT cat_weights_json,kw_weights_json,plat_weights_json FROM user_prefs WHERE user_id=?`)
...
// expose VAPID public key for the frontend
