...
app.get('/api/v1/admin/queries.txt', async (req, res) => {
  if (!isAdmin(req)) return res.status(401).end('unauthorized');
  const limit = Number(req.query.limit || 60);
  const r = await q<{ value: string }>(
    `SELECT value FROM source_queries
       WHERE kind='cse' AND active=true
       ORDER BY priority DESC, last_success DESC NULLS LAST, id ASC
       LIMIT $1`, [limit]);
  res.setHeader('content-type','text/plain');
  res.send(r.rows.map(x=>x.value).join('\n'));
});

app.post('/api/v1/enrich-one', async (req, res) => {
  if (!isAdmin(req)) return res.status(401).json({ ok: false, error: 'unauthorized' });
  const id = Number((req.query.id as string) || req.body?.id);
  if (!id || Number.isNaN(id)) return res.status(400).json({ ok: false, error: 'bad id' });
...
app.post('/api/v1/events', async (req, res) => {
  ...
  if (type === 'mute_domain' && userId) {
    const host = meta?.domain as string | undefined;
    if (host) await q(
      `UPDATE app_user SET user_prefs =
        jsonb_set(COALESCE(user_prefs,'{}'::jsonb), '{muteDomains}',
          COALESCE(user_prefs->'muteDomains','[]'::jsonb) || to_jsonb($2::text))
        WHERE id=$1`, [userId, host]);
  }
  res.json({ ok: true });
});
...
app.get('/api/v1/leads', async (req, res) => {
  ...
  const limit = 40;
  const r = await q(
    `SELECT id, cat, kw, platform, fit_user, heat, source_url,
            title, snippet, ttl, state, created_at
       FROM lead_pool
  ...
  const weights: Weights = (wRow.rows[0]?.weights as Weights) ||
  ...
  if (leads.length < 6) {
    ...
    const demos = Array.from({ length: missing }).map((_, i) => ({
      id: -(i+1), cat:'demo', kw:['packaging'], platform:'demo', fit_user:60, heat:60,
      source_url:'https://example.com/demo', title:'Sample Lead',
      snippet:'Demo card while ingest warms up', ttl: toISO(nowPlusMinutes(60)),
      state:'available', created_at: new Date().toISOString(), _score:0
    }));
    leads = [...leads, ...demos];
  }
...
app.post('/api/v1/claim', async (req, res) => {
  ...
  await q(`INSERT INTO event_log (user_id, lead_id, event_type, meta) VALUES ($1,$2,'claim','{}')`,
    [userId, leadId]);
