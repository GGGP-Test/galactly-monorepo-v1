...
const limit = 20;
const r = await q(
`SELECT id, cat, kw, platform, fit_user, heat, source_url, title, snippet, ttl, state
FROM lead_pool WHERE state='available' ORDER BY created_at DESC LIMIT $1`, [limit]
);
let leads = r.rows;
...
source_url: 'https://example.com/demo', title: 'Sample Lead', snippet: 'Demo card while ingest warms up', ttl: toISO(nowPlusMinutes(60)), state: 'available'
}));
leads = [...leads, ...demos];
}
res.json({ ok: true, leads, nextRefreshSec });
});
...
res.json({ ok: true, windowId, reservedForSec: 120, reveal: { /* add reveal hints later */ } });
});
...
// Status (simple heuristics for now)
