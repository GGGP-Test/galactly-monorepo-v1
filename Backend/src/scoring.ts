...
export type LeadRow = {
  id: number; platform: string | null; source_url: string;
  title: string | null; snippet: string | null; created_at?: string | null;
  cat?: string | null; kw?: string[] | null;
};
...
function recencyScore(createdAtIso?: string | null): number {
  ...
  return Math.max(0,1-ageMin/240);
}
function intentScore(title?: string | null, snippet?: string | null, intents?: string[]): number {
  const keys = intents?.length ? intents :
    (process.env.INTENT_KEYWORDS || 'need,looking for,rfp,rfq,quote,recommend,supplier,who can supply,sourcing')
...
function platformWeight(w: Weights, platform?: string | null): number {
  const p=(platform||'').toLowerCase(); return p?(w.platforms[p] ?? 0.8):0.5;
}
function domainQuality(w: Weights, url: string): number {
  const host=domainFromUrl(url); if(!host) return 0;
  if (w.badDomains?.some(b=>host.endsWith(b))) return -1;
  return 0.5;
}
...
export function computeScore(lead: LeadRow, w: Weights, prefs?: UserPrefs): number {
  const r=recencyScore(lead.created_at),
        pf=platformWeight(w, lead.platform),
        dm=domainQuality(w, lead.source_url),
        inx=intentScore(lead.title, lead.snippet),
        hist=0,
        uf=userFitScore(lead, prefs);
  if (uf.muted) return -1e9;
  const c=w?.coeffs||{recency:0.4,platform:1.0,domain:0.5,intent:0.6,histCtr:0.3,userFit:1.0};
