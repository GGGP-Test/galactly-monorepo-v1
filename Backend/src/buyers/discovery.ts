...
async function fetchLeadsFromGoogle(location: string): Promise<Array<{ name: string; url: string; location: string; evidence: string }>> {
  ...
  return [
    { name: "Demo Lead 1", url: "https://example.com", location: "New York", evidence: "Google Search: packaging supplier New York" },
    { name: "Demo Lead 2", url: "https://example.com", location: "Toronto", evidence: "Google Search: packaging supplier Toronto" },
  ];
}
...
async function fetchLeadsFromKompass(location: string): Promise<Array<{ name: string; url: string; location: string; evidence: string }>> {
  ...
  //Replace with actual implementation
  return [
    { name: "Demo Lead 3", url: "https://example.com", location: "Los Angeles", evidence: "Kompass Search: packaging supplier Los Angeles" },
  ];
}
